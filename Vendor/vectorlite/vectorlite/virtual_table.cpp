...
static int InitVirtualTable(bool load_from_file, sqlite3* db, void* pAux,
  ...
  try {
    auto* connection_state = static_cast<ConnectionState*>(pAux);
    auto vtab = new VirtualTable(std::move(*vector_space), *index_options,
                                 index_file_path, argv[2], connection_state);
    *ppVTab = vtab;
    if (connection_state != nullptr) {
      connection_state->RegisterTable(argv[2], vtab);
    }
...
int VirtualTable::Destroy(sqlite3_vtab* pVTab) {
  ...
  if (vtab->connection_state() != nullptr) {
    vtab->connection_state()->UnregisterTable(vtab->table_name(), vtab);
  }
