...
private enum Gemma4Error: LocalizedError {
    ...
    case imagePromptPlaceholderMismatch(imageCount: Int, rawImageTokenCount: Int)
    ...
    var errorDescription: String? {
        ...
        case .imagePromptPlaceholderMismatch(let imageCount, let rawImageTokenCount):
            return
                "Gemma4 image prompt mismatch: received \(imageCount) images, but the prompt contains \(rawImageTokenCount) raw image-token placeholders."
        }
    }
}

func gemma4ExpandPromptImageTokens(
    _ promptTokens: [Int],
    imageCount: Int,
    imageTokenId: Int,
    boiTokenId: Int,
    ...
    let rawImageTokenCount = promptTokens.filter { $0 == imageTokenId }.count
    guard imageCount > 0, rawImageTokenCount > 0,
        rawImageTokenCount.isMultiple(of: imageCount)
    else {
        throw Gemma4Error.imagePromptPlaceholderMismatch(
            imageCount: imageCount,
            rawImageTokenCount: rawImageTokenCount
        )
    }

    let rawTokensPerImagePlaceholder = rawImageTokenCount / imageCount
    var skippedRawImageTokens = 0
    return promptTokens.reduce(into: []) { expandedTokens, token in
        if token == imageTokenId {
            guard skippedRawImageTokens == 0 else {
                skippedRawImageTokens =
                    (skippedRawImageTokens + 1) % rawTokensPerImagePlaceholder
                return
            }
            expandedTokens.append(boiTokenId)
            expandedTokens.append(
                contentsOf: Array(repeating: imageTokenId, count: imageSeqLength))
            if let eoiTokenId {
                expandedTokens.append(eoiTokenId)
            }
            skippedRawImageTokens =
                rawTokensPerImagePlaceholder == 1 ? 0 : 1
        } else {
...
func gemma4ValidateImageTokenCount(
    visionBatchSize: Int,
    visionTokensPerImage: Int,
    promptImageTokenCount: Int
) throws {
    let expectedVisionTokens = visionBatchSize * visionTokensPerImage
    if promptImageTokenCount != expectedVisionTokens {
        throw Gemma4Error.imageTokenCountMismatch(
            expectedVisionTokens: expectedVisionTokens,
            actualPromptTokens: promptImageTokenCount)
    }
}
...
public final class Gemma4: Module, VLMModel, KVCacheDimensionProvider {
    ...
    private func getInputEmbeddings(
        ...
        let imageMask = inputIds .== config.imageTokenId
        let promptImageTokenCount = imageMask.asType(.int32).sum().item(Int.self)
        try gemma4ValidateImageTokenCount(
            visionBatchSize: imageFeatures.dim(0),
            visionTokensPerImage: imageFeatures.dim(1),
            promptImageTokenCount: promptImageTokenCount
        )
...
public struct Gemma4ProcessorConfiguration: Codable, Sendable {
    ...
    public init(from decoder: any Swift.Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        let nested = try decoder.container(keyedBy: NestedCodingKeys.self)
        let imageProcessor = try nested.decodeIfPresent(
            ImageProcessorConfiguration.self,
            forKey: NestedCodingKeys.imageProcessor
        )
        processorClass = try c.decode(String.self, forKey: CodingKeys.processorClass)
        let topLevelDoNormalize = try c.decodeIfPresent(
            Bool.self, forKey: CodingKeys.doNormalize)
        let topLevelImageMean = try c.decodeIfPresent(
            [CGFloat].self, forKey: CodingKeys.imageMean)
        let topLevelImageStd = try c.decodeIfPresent(
            [CGFloat].self, forKey: CodingKeys.imageStd)
        let topLevelImageSeqLength = try c.decodeIfPresent(
            Int.self, forKey: CodingKeys.imageSeqLength)
        let topLevelSize = try c.decodeIfPresent(
            Gemma3ProcessorConfiguration.ImageSize.self, forKey: CodingKeys.size)
        let topLevelImageTokenId = try c.decodeIfPresent(
            Int.self, forKey: CodingKeys.imageTokenId)
        let topLevelBoiTokenId = try c.decodeIfPresent(
            Int.self, forKey: CodingKeys.boiTokenId)
        let topLevelEoiTokenId = try c.decodeIfPresent(
            Int.self, forKey: CodingKeys.eoiTokenId)

        doNormalize = imageProcessor?.doNormalize ?? topLevelDoNormalize ?? false
        imageMean = imageProcessor?.imageMean ?? topLevelImageMean ?? [0.5, 0.5, 0.5]
        imageStd = imageProcessor?.imageStd ?? topLevelImageStd ?? [0.5, 0.5, 0.5]
        imageSeqLength = imageProcessor?.imageSeqLength ?? topLevelImageSeqLength ?? 280
        size = imageProcessor?.size ?? topLevelSize
        imageTokenId = imageProcessor?.imageTokenId ?? topLevelImageTokenId ?? 258_880
        boiTokenId = imageProcessor?.boiTokenId ?? topLevelBoiTokenId ?? 255_999
        eoiTokenId = imageProcessor?.eoiTokenId ?? topLevelEoiTokenId ?? 258_882
    }
