...
public func createLeftPaddedCausalMask(
    n: Int,
    offset: Int,
    ...
    linds = linds[0..., .newAxis]
    rinds = rinds[.newAxis]
    let padding = leftPadding[0..., .newAxis, .newAxis, .newAxis]
    let queryIsPadding = linds .< padding
    var mask = linds .>= rinds
    ...
    return mask & ((rinds .>= padding) | queryIsPadding)
}
