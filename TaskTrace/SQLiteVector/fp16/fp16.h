...
static inline uint32_t fp16_ieee_to_fp32_bits(uint16_t h) {
	 ...
	 */
	return sign | ((((nonsign << renorm_shift >> 3) + ((UINT32_C(0x70) - renorm_shift) << 23)) | (uint32_t)inf_nan_mask) & ~(uint32_t)zero_mask);
}
...
static inline uint32_t fp16_alt_to_fp32_bits(uint16_t h) {
	 ...
	 */
	return sign | (((nonsign << renorm_shift >> 3) + ((UINT32_C(0x70) - renorm_shift) << 23)) & ~(uint32_t)zero_mask);
}
