...
#define BIT_BYTES_FOR_DIM(_dim)                     ((((size_t)(_dim)) + 7u) / 8u)

#define INT64_TO_INT8PTR(_val, _ptr)                do { \
                                                    (_ptr)[0] = (uint8_t)(((_val) >> 0)  & 0xFF); \
                                                    (_ptr)[1] = (uint8_t)(((_val) >> 8)  & 0xFF); \
                                                    (_ptr)[2] = (uint8_t)(((_val) >> 16) & 0xFF); \
                                                    (_ptr)[3] = (uint8_t)(((_val) >> 24) & 0xFF); \
                                                    (_ptr)[4] = (uint8_t)(((_val) >> 32) & 0xFF); \
                                                    (_ptr)[5] = (uint8_t)(((_val) >> 40) & 0xFF); \
                                                    (_ptr)[6] = (uint8_t)(((_val) >> 48) & 0xFF); \
                                                    (_ptr)[7] = (uint8_t)(((_val) >> 56) & 0xFF); \
                                                    } while(0)
...
#define KEY_MATCH(_k)                               (key_len == (int)sizeof(_k) - 1 && strncasecmp(key, _k, (size_t)key_len) == 0)
...
static char *sqlite_strdup (const char *str) {
    ...
    char *result = (char*)sqlite3_malloc64((sqlite3_uint64)len);
    if (result) memcpy(result, str, len);
    
    return result;
}

static void *sqlite_memdup (const void *v, size_t len) {
    if (!v) return NULL;
    
    void *result = (void *)sqlite3_malloc64((sqlite3_uint64)len);
...
static void quantize_binary (const float *input, uint8_t *output, int dim, bool is_binary_mean) {
    ...
    if (is_binary_mean) {
        ...
        threshold = sum / (float)dim;
    }

    memset(output, 0, BIT_BYTES_FOR_DIM(dim));
    for (int i = 0; i < dim; i++) {
...
static size_t vector_bytes_for_dim (vector_type type, int dim) {
    ...
    if (type == VECTOR_TYPE_BIT) {
        return BIT_BYTES_FOR_DIM(dim);   // Ceil division: pack 8 dimensions per byte
    }
...
static int vector_serialize_quantization (sqlite3 *db, const char *table_name, const char *column_name, uint32_t nrows, uint8_t *data, sqlite3_uint64 data_size, int64_t min_rowid, int64_t max_rowid) {
    ...
    rc = sqlite3_bind_int64(vm, 3, (sqlite3_int64)nrows);
    if (rc != SQLITE_OK) goto vector_serialize_quantization_cleanup;
    
    rc = sqlite3_bind_blob64(vm, 4, (const void *)data, data_size, SQLITE_STATIC);
    if (rc != SQLITE_OK) goto vector_serialize_quantization_cleanup;
...
static int vector_rebuild_quantization (sqlite3_context *context, const char *table_name, const char *column_name, table_context *t_ctx, vector_qtype qtype, uint64_t max_memory, uint32_t *count) {
    ...
    size_t quant_bytes = (qtype == VECTOR_QUANT_1BIT) ? BIT_BYTES_FOR_DIM(dim) : ((size_t)dim) * sizeof(uint8_t);
    ...
    while (1) {
        ...
        if (qtype == VECTOR_QUANT_1BIT) {
            ...
            switch (type) {
                ...
                case VECTOR_TYPE_BIT: memcpy(data, blob, BIT_BYTES_FOR_DIM(dim)); break; // Already binary
            }
        } else {
            ...
            switch (type) {
                ...
                case VECTOR_TYPE_BIT: memcpy(data, blob, BIT_BYTES_FOR_DIM(dim)); break; // BIT to 8-bit: just copy
            }
        }
        ...
        data += (qtype == VECTOR_QUANT_1BIT) ? BIT_BYTES_FOR_DIM(dim) : ((size_t)dim) * sizeof(uint8_t);
        max_rowid = rowid;
        ++n_processed;
        ++tot_processed;
        
        if (n_processed == max_vectors) {
            sqlite3_uint64 batch_size = (sqlite3_uint64)(data - original);
...
static void vector_quantize_preload (sqlite3_context *context, int argc, sqlite3_value **argv) {
    ...
    int counter = 0;
    void *buffer = (void *)sqlite3_malloc64((sqlite3_uint64)required);
    if (!buffer) {
...
static void *vector_from_json (sqlite3_context *context, sqlite3_vtab *vtab, vector_type type, const char *json, int *size, int dimension) {
    ...
    size_t estimated_items = (size_t)(estimated_count + 1);
    size_t alloc = (type == VECTOR_TYPE_BIT) ? ((estimated_items + 7u) / 8u) : (estimated_items * item_size);
    blob = sqlite3_malloc64((sqlite3_uint64)alloc);
    if (!blob) {
    ...
    while (*p) {
        ...
        // check bounds
        int max_count = (type == VECTOR_TYPE_BIT) ? (int)(alloc * 8u) : (int)(alloc / item_size);
        if (count >= max_count) {
            sqlite3_free(blob);
    ...
    if (size) {
        size_t size_bytes = (type == VECTOR_TYPE_BIT) ? BIT_BYTES_FOR_DIM(count) : ((size_t)count) * item_size;
        *size = (int)size_bytes;
    }
    return blob;
}
...
static void vector_as_type (sqlite3_context *context, vector_type type, int argc, sqlite3_value **argv) {
    ...
    if (value_type == SQLITE_BLOB) {
        if (type == VECTOR_TYPE_BIT) {
            ...
            size_t item_size = vector_type_to_size(type);
            if (((size_t)value_size % item_size) != 0u) {
                context_result_error(context, SQLITE_ERROR,
                                     "Invalid BLOB size for format '%s': size must be a multiple of %d bytes",
                                     vector_type_to_name(type), (int)item_size);
                return;
            }
        }
    ...
    if (value_type == SQLITE_TEXT) {
        ...
        if (print_dim == 0) print_dim = (type == VECTOR_TYPE_BIT) ? value_size * 8 : (int)((size_t)value_size / vector_type_to_size(type));
...
static int vCursorFilterCommon (sqlite3_vtab_cursor *cur, int idxNum, const char *idxStr, int argc, sqlite3_value **argv, const char *fname, vcursor_run_callback run_callback, vcursor_sort_callback sort_callback, vcursor_run_callback stream_callback, bool quantized) {
    ...
    if (c->row_count != k) {
        ...
        c->rowids = (int64_t *)sqlite3_malloc64((sqlite3_uint64)((size_t)k * sizeof(int64_t)));
        if (c->rowids == NULL) {
            if (vector_allocated) sqlite3_free((void *)vector);
            return SQLITE_NOMEM;
        }
        ...
        c->distance = (double *)sqlite3_malloc64((sqlite3_uint64)((size_t)k * sizeof(double)));
        if (c->distance == NULL) {
            if (vector_allocated) sqlite3_free((void *)vector);
            return SQLITE_NOMEM;
        }
    ...
    memset(c->rowids, 0, ((size_t)k) * sizeof(int64_t));
...
static int vFullScanRun (sqlite3 *db, vFullScanCursor *c, const void *v1, int v1size) {
    ...
    int dist_size = (vt == VECTOR_TYPE_BIT) ? (int)BIT_BYTES_FOR_DIM(dimension) : dimension;
...
static int vQuantRunMemory(vFullScanCursor *c, uint8_t *v, vector_qtype qtype, int dim) {
    ...
    const size_t rowid_size = sizeof(int64_t);
    const size_t vector_size = (qtype == VECTOR_QUANT_1BIT) ? BIT_BYTES_FOR_DIM(dim) : ((size_t)dim) * sizeof(uint8_t);
    const size_t total_stride = rowid_size + vector_size;
    ...
    for (int i = 0; i < counter; ++i) {
        const uint8_t *current_data = data + (((size_t)i) * total_stride);
        const uint8_t *vector_data = current_data + rowid_size;
...
static int vQuantRun (sqlite3 *db, vFullScanCursor *c, const void *v1, int v1size) {
    ...
    size_t alloc_size = (qtype == VECTOR_QUANT_1BIT) ? BIT_BYTES_FOR_DIM(dimension) : ((size_t)dimension) * sizeof(int8_t);
    uint8_t *v = (uint8_t *)sqlite3_malloc64((sqlite3_uint64)alloc_size);
    ...
    if (qtype == VECTOR_QUANT_1BIT) {
        ...
        switch (type) {
            ...
            case VECTOR_TYPE_BIT: memcpy(v, v1, BIT_BYTES_FOR_DIM(dimension)); break; // Already binary
        }
    } else {
        // 8-bit quantization (U8BIT or S8BIT)
        switch (type) {
            ...
            case VECTOR_TYPE_BIT: memcpy(v, v1, BIT_BYTES_FOR_DIM(dimension)); break; // BIT to 8-bit: just copy
        }
    }
    ...
    const size_t rowid_size = sizeof(int64_t);
    const size_t vector_size = (qtype == VECTOR_QUANT_1BIT) ? BIT_BYTES_FOR_DIM(dimension) : ((size_t)dimension) * sizeof(uint8_t);
    const size_t total_stride = rowid_size + vector_size;
...
static int vStreamScanCursorRun (sqlite3 *db, vFullScanCursor *c, const void *v1, int v1size) {
    ...
    void *v = sqlite_memdup(v1, (size_t)v1size);
    if (!v) return SQLITE_NOMEM;
...
static int vStreamQuantCursorRun (sqlite3 *db, vFullScanCursor *c, const void *v1, int v1size) {
    ...
    c->stream.vector = (void *)v;
    c->stream.vsize = (qtype == VECTOR_QUANT_1BIT) ? (int)BIT_BYTES_FOR_DIM(dimension) : (int)(((size_t)dimension) * sizeof(int8_t));
    c->stream.vdim = dimension;
