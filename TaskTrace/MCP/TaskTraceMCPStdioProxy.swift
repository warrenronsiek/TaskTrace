...
@MainActor
final class TaskTraceMCPStdioProxyController {
    ...
    func start() {
        ...
        task = Task {
            ...
            if exitCode != 0 {
                Logger(subsystem: "com.tasktrace.TaskTrace", category: "mcp-proxy").error(
                    "mcp stdio proxy exited with code=\(exitCode, privacy: .public)"
                )
            }
...
struct TaskTraceMCPStdioProxy: Sendable {
    ...
    init(
        socketPath: String = ProcessInfo.processInfo.environment["TASKTRACE_MCP_SOCKET_PATH"] ?? Vars.mcpBrokerSocketPath,
    ...
    nonisolated func run() async -> Int32 {
        ...
        logger.log("mcp stdio proxy connected socket=\(socketPath, privacy: .public)")
    ...
    nonisolated private static func copy(from sourceFileDescriptor: Int32, to destinationFileDescriptor: Int32) -> Int32 {
...
@MainActor
final class TaskTraceMCPHelperProcessRegistry {
    static let shared = TaskTraceMCPHelperProcessRegistry()

    private var processIdentifiers: Set<Int32> = []

    func register(_ processIdentifier: Int32) {
        processIdentifiers.insert(processIdentifier)
    }

    func unregister(_ processIdentifier: Int32) {
